      ...
      .reduce(
        (accumulator, heading) => {
          ...
          if (
            acc &&
            acc[acc.length - 1] &&
            acc[acc.length - 1].level < heading.split(" ")[0].length
          ) {
            if (
              acc[acc.length - 1].subheading[acc[acc.length - 1].subheading.length - 1] &&
              acc[acc.length - 1].subheading[acc[acc.length - 1].subheading.length - 1].subLevel <
                heading.split(" ")[0].length
            ) {
              acc[acc.length - 1].subheading[
                acc[acc.length - 1].subheading.length - 1
              ].subheading.push(makeSubheading(inputObj));
            } else {
...
const mdxOptions = {
  ...
  remarkPlugins: [
    ...
    [
      ...
      {
        behavior: "wrap",
        linkProperties: { ariaHidden: true, tabIndex: 0, className: ["autolink-heading"] },
      },
    ],
